...
def knn():
    ...
    def euclidean_dist(x, y):
        return np.sqrt((st.session_state['height-knn'] - x) ** 2 + (st.session_state['weight-knn'] - y) ** 2)
    ...
    # if 'height-knn' not in st.session_state:
    #     st.session_state['height-knn'] = 165.
    # if 'weight-knn' not in st.session_state:
    #     st.session_state['weight-knn'] = 80.
    ...
    with col2:
        ...
        k_indices = np.argsort(distances)[:st.session_state['k-knn']]
        ...
        with col21:
            st.metric('Male', f"{k_labels.count('Male')}/{st.session_state['k-knn']}")
        with col22:
            st.metric('Female', f"{k_labels.count('Female')}/{st.session_state['k-knn']}")
    ...
    with col1:    
        ...
        fig.add_trace(go.Scatter(
            x = [st.session_state['height-knn']],
            y = [st.session_state['weight-knn']],
            mode = 'markers',
            marker_color = 'black',
            marker_symbol = 'x',
            marker_size = 10,
